...
scaledData = (countrydata - mean(countrydata,2)) ./ std(countrydata,0,2);
...
Cov  = (scaledData - mean(scaledData,2)) * (scaledData - mean(scaledData,2))' / (N-1);
...
PC1 = E(:,1)' * scaledData; % PC1 is scaledData projected to first PC
PC2 = E(:,2)' * scaledData; % PC2 is scaledData projected to second PC
...
title({'Projection onto the first two PCs', ...
...
% Get data for class 1 and class 3
...
% their sum
C1 = (X1 - mean(X1, 2)) * (X1 - mean(X1, 2))' / (N1 - 1);
C3 = (X3 - mean(X3, 2)) * (X3 - mean(X3, 2))' / (N3 - 1);
Ctot = C1 + C3;

% Calculate the direction of maximal separation of the two classes.
% Normalize the vector to length 1.
W = Ctot \ (mean(X1, 2) - mean(X3, 2));
W = W / norm(W,1);
...
axis image;
ylim([-eps,eps]);
xlim([min([XP1,XP3])-0.1,max([XP1,XP3])+0.1]);
box on;
...
title({'This is the same as the first plot,', 'only separated in y-direction to improve visibility'});
xlabel('Value when projecting onto W');
